...
export const ResultsTable = ({ data }: { data?: ConciliationData }) => {
  ...
  const invoices: Invoice[] = data?.conciliados?.length
    ? [
    ...
    <div className="rounded-2xl border bg-card backdrop-blur-sm p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm-items-center mb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            className="px-3 py-1 rounded text-xs font-medium bg-primary/5 text-primary hover:bg-primary/10 transition-colors"
            data-filter="all"
          >Todas</button>
          <button
            className="px-3 py-1 rounded text-xs font-medium bg-success/5 text-success hover:bg-success/10 transition-colors"
            data-filter="conciliada"
          >Conciliadas</button>
          <button
            className="px-3 py-1 rounded text-xs font-medium bg-warning/5 text-warning hover:bg-warning/10 transition-colors"
            data-filter="pendiente"
          >Pendientes</button>
          <button
            className="px-3 py-1 rounded text-xs font-medium bg-error/5 text-error hover:bg-error/10 transition-colors"
            data-filter="sin_factura"
          >Sin Factura</button>
        </div>
      </div>
            ...
            {filteredData.map((item) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <path
                          ...
                          strokeWidth="2"
                          d="M9 19V6a2 2 0 00-2-2H5a2 2 0 00-2 2v13c0 2 2 2 2 2h14a2 2 0 002-2v-7m-5-3l-3.138 5.326L16.07 21.375c.538.434.21.927-.262 1.073l-4.893-.696L5.37 21.85c-.429.53-.972.225-1.03-.262l-1.158-4.983L.829 9.36l4.465-3.536a1.967 1.967 0 01.03-.651zm0 0l3.138-5.326L8.93 9.375c-.538-.434-.21-.927.262-1.073l4.893.696L18.63 1.65c.429-.53.21-.927-.262-1.073L13.97 2.38l-4.465 3.536a1.967 1.967 0 01-.03.651zm0 0l3.138-5.326L8.93 9.375c-.538-.434-.21-.927.262-1.073l4.893.696L18.63 1.65c.429-.53.21-.927-.262-1.073L13.97 2.38l-4.465 3.536a1.967 1.967 0 01-.03.651z"
                        />
                      </svg>
                      <span>{item.id}</span>
                      {tipoLabel && (
                        <span className="text-xs ms-1 bg-primary/20 text-primary rounded px-1 py-0.5">
                          {tipoLabel}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="text-left text-sm font-medium px-6 py-3">
                    <button
                      className="px-2 py-0.5 rounded text-xs bg-primary/5 text-primary hover:bg-primary/10 transition-colors"
                    >
                      Ver
                    </button>
                  </td>
